...
class DataCrypto
{
    ...
    public function sysTimeStampByLinkToken($LinkToken){
        $tokenKey = self::LINK_TOKEN_KEY;
        $timeStamp = $this->_sysStrDecrypt($LinkToken, $tokenKey);
        return $timeStamp;
    }

    //@TODO 判断linkToken是否在有效时间内，与现货对接
    public function _tmpSysLinkTokenIsEnable($LinkToken){
        $howLongSecond = 60;
        $nowTimeStamp = time();
        $timeStamp = $this->sysTimeStampByLinkToken($LinkToken);
        //dd(date('Y-m-d H:i:s', $timeStamp));
        if($nowTimeStamp - $timeStamp <= $howLongSecond){
            return true;
        }else{
            return false;
            //errorCode = 10007; errorMessage = "系统检测到通信密钥值（token）过期，请重新操作！";
            //errorCode = 10006; errorMessage = "系统检测到通信密钥值（token）不正确，请重新操作或者联系管理人员！";
        }
    }
